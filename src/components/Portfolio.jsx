          ...
          {filteredProjects.map((project) => (
            ...
            >
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <h3 className="text-lg font-bold text-white mb-2 group-hover:text-cyan-400 transition">{project.title[lang]}</h3>
                <p className="text-gray-400 text-xs leading-relaxed mb-4 flex-grow">{project.description[lang]}</p>
        ...
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl max-w-2xl w-full overflow-hidden relative shadow-2xl">
            ...
            <div className="h-64 sm:h-80 overflow-hidden bg-black flex items-center justify-center">
              <img src={selectedImage.image} alt={selectedImage.title[lang]} className="w-full h-full object-contain" />
            </div>
            <div className="p-6">
              <span className="state text-xs text-cyan-400 font-mono font-semibold">{selectedImage.category} • {selectedImage.year}</span>
              <h3 className="text-2xl font-bold text-white mt-1 mb-3">{selectedImage.title[lang]}</h3>
              <p className="text-gray-300 text-sm mb-6">{selectedImage.description[lang]}</p>
